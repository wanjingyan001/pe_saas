...
class PayDialog {
    companion object {
        ...
        fun showPayFareDialog(context: Context, callBack: AllPayCallBack) {
            ...
            val dialog = MaterialDialog.Builder(context)
                    .theme(Theme.DARK)
                    .customView(R.layout.layout_pay_fare, false)
                    .canceledOnTouchOutside(false)
                    .build()
            dialog.window.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))
            dialog.show()
            ...
            tv_pay.clickWithTrigger {
                //去支付
                dialog.dismiss()
                callBack.pay(5, 1, pay_type, FarePlace, tv_per_balance, iv_pre_select,
                        tv_bus_balance, iv_bus_select, tv_per_title, tv_bus_title, dialog)
            }
        }
