...
/**
 * Created by sogubaby on 2018/7/2.
 */
...
public class LunarUtils {
    ...
    static SimpleDateFormat chineseDateFormat = new SimpleDateFormat("yyyyMMdd", Locale.CHINA);
     ...
     * @return
     */
    public LunarBean getLunarDate(String date, boolean isday) {
        ...
        try {
            nowaday = chineseDateFormat.parse(date);
        } catch (ParseException e) {
            e.printStackTrace(); // To change body of catch statement use
            // Options | File Templates.
        }
        ...
        if (!isday) {
            ...
            for (int i = 0; i < solarHoliday.length; i++) {
                ...
                int month_log = Integer.parseInt(date.substring(4, 6));
                int day_log = Integer.parseInt(date.substring(6, 8));
            ...
            for (int i = 0; i < lunarHoliday.length; i++) {
                ...
                if (ld.trim().equals(lmd.trim())) {
                    return new LunarBean(ldv, true);
                }
            }
        }
        if (day == 1)
            return new LunarBean(chineseNumber[month - 1] + "月", false);
        else
            return new LunarBean(getChinaDayString(day), false);
